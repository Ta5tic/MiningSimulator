	...
	public class HighlightingBase : MonoBehaviour
	{
		...
		public void RenderHighlighting(RenderTexture frameBuffer)
		{
			...
			if (isDepthAvailable != depthAvailable)
			{
				...
				if (isDepthAvailable)
				{
					Debug.LogWarning("HighlightingSystem : Framebuffer depth data is available back again and now will be used to occlude highlighting. Highlighting occluders disabled.");
				}
				else
				{
			...
			highlightingBuffer = RenderTexture.GetTemporary(w, h, depth, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, aa);
			...
			RenderTexture buffer = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1);
			RenderTexture buffer2 = RenderTexture.GetTemporary(width, height, 0, RenderTextureFormat.ARGB32, RenderTextureReadWrite.Linear, 1);
			if (!buffer.IsCreated())
			{
